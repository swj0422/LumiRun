        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="log in logs" :key="log.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ log.updated_at ? formatDate(log.updated_at) : '-' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <button
                  @click="editLog(log)"
              ...
              <div v-if="growthTags.length > 0" class="mt-2">
                <div class="flex flex-wrap gap-2">
                  <button
                    ...
                    type="button"
                    @click="
                      ((growthForm.reason = tag.name),
                      (growthForm.score = Number(tag.description)))
                    "
    ...
    >
      <div class="bg-white rounded-lg p-6 w-full max-w-lg">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1"
              >选择班级</label
            >
            <select v-model="batchForm.class_id" required class="input">
              <option value="">请选择班级</option>
              <option v-for="cls in classes" :key="cls.id" :value="cls.id">
                {{ cls.session || '' }}级 {{ cls.class_name }}班 -
                {{ cls.school_name || '' }}
          ...
          <div class="flex gap-2">
            ...
            <button
              type="button"
              @click="($refs.batchFileInput as HTMLInputElement).click()"
              class="btn-secondary"
            >
              {{ batchFileName || '选择文件' }}
            </button>
        ...
        <div class="flex justify-end space-x-3 mt-6">
          ...
          <button type="button" @click="handleBatchImport" class="btn-primary">
            导入
          </button>
        </div>
      </div>
...
const fetchClasses = async () => {
  try {
    const data: any = await request.get('/api/v1/classes/');
...
const fetchLogs = async () => {
  try {
    ...
    if (selectedSchool.value) {
      params.append('school_name', selectedSchool.value);
    }
    if (selectedSession.value) {
      params.append('session', selectedSession.value);
    }
    ...
    console.log('请求成长值记录:', `/api/v1/growth/logs?${params.toString()}`);
    const data: any = await request.get(`/api/v1/growth/logs?${params.toString()}`);
    console.log('获取到的成长值记录:', data);
    ...
    if (response && response.log) {
      ...
      if (index !== -1) {
        ...
        logs.value[index] = {
          ...
          change_score: updatedLog.change_value,
          reason: updatedLog.reason,
          updated_at: new Date().toISOString(),
          input_type_name: '手动录入',
          operator_name: operatorName,
          action: '添加', // 保持原记录的操作类型为添加
        };
      }
    }
...
const fetchStudents = async () => {
  try {
    const data = await request.get('/api/v1/students/teacher-students');
    if (Array.isArray(data)) {
      students.value = data
        .map((item: any) => ({
          id: item.id,
          real_name: item.name_in_class || item.real_name,
...
const handleAddGrowth = async () => {
  try {
    ...
    // 保存当前表单数据，用于后续添加到列表
    const { student_id, score, reason } = growthForm.value;

    // 获取选中的学员信息
    const selectedStudent = students.value.find(
      (s) => String(s.id) === student_id
    );
    if (!selectedStudent) {
    ...
    const response = await request.post('/api/v1/growth/record', {
      ...
      class_id: parseInt(growthForm.value.class_id),
      change_score: score,
      reason: reason,
      input_type: 1,
    });
    ...
    // 获取班级信息
    const selectedClass = classes.value.find(
      (cls) => String(cls.id) === growthForm.value.class_id
    );
    const className = selectedClass ? selectedClass.class_name : '';
...
const downloadBatchTemplate = async () => {
  ...
  try {
    const data = await request.get('/api/v1/students/teacher-students', {
      params: {
        ...
        limit: 9999  // 使用大的limit获取所有学员
      }
    });
    if (Array.isArray(data)) {
      allStudents = data.map((item: any) => ({
        id: item.id,
        real_name: item.name_in_class || item.real_name,
        student_no_in_class: item.student_no_in_class || '',
        class_name: item.class_name,
      ...
      allStudents = data.items.map((item: any) => ({
        id: item.id,
        real_name: item.name_in_class || item.real_name,
        student_no_in_class: item.student_no_in_class || '',
        class_name: item.class_name,
  ...
  console.log('获取到的学员数量:', allStudents.length);
  ...
  // 过滤出该班级的学员
  console.log('所有学员:', allStudents);
  console.log('选择的班级信息:', selectedClassInfo);

  const classStudents = allStudents.filter((student: any) => {
    // 根据学校、届和班级名称进行精确匹配
    return (
      selectedClassInfo &&
      student.class_name === selectedClassInfo.class_name &&
      student.session === selectedClassInfo.session &&
      student.school_name === selectedClassInfo.school_name
    );
  });
  ...
  if (growthTags.length > 0) {
    ...
    console.log('数据验证添加成功');
  } else {
    console.log('没有找到成长标签');
  }

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, '成长值导入模板');

  // 添加成长原因参考工作表
  const reasonSheetData = [
    ['成长原因参考'],
    ['黄色背景行不可删除或变更，否则无法导入'],
    ['以下是系统中已创建的成长值标签，您可以在导入模板中使用这些标签：'],
    ['成长原因', '成长值'],
  ];

  // 添加所有成长标签
  const growthTagsWithValues = tags.value
    .filter((t) => t.type === 'growth')
    .map((t) => [t.name, t.description]);
  growthTagsWithValues.forEach((tag) => {
    reasonSheetData.push(tag);
  });

  const reasonWs = XLSX.utils.aoa_to_sheet(reasonSheetData);

  // 设置列宽
  reasonWs['!cols'] = [{ wch: 30 }, { wch: 10 }];

  // 设置黄色背景
  for (let col = 0; col < 2; col++) {
    for (let row = 0; row < 3; row++) {
      const cellAddress = XLSX.utils.encode_cell({ r: row, c: col });
      if (reasonWs[cellAddress]) reasonWs[cellAddress].s = yellowStyle;
    }
  }
  ...
  for (let col = 0; col < reasonSheetData[0].length; col++) {
    const cellAddress = XLSX.utils.encode_cell({ r: 0, c: col });
    if (reasonWs[cellAddress]) reasonWs[cellAddress].s = headerStyle;
  }

  XLSX.utils.book_append_sheet(wb, reasonWs, '成长原因参考');

  XLSX.writeFile(wb, `成长值导入模板_${selectedClassInfo.class_name}.xlsx`);
};
...
const handleBatchImport = debounce(async () => {
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (records.length === 0) {
        alert(
          '未解析到有效的成长值记录，请确保学员学号、姓名和标签名称都已填写'
        );
        return;
      }
...
onMounted(async () => {
  ...
  if (studentName) {
    searchKeyword.value = studentName;
    // 检查学员是否存在且状态正常
    const studentExists = students.value.some(
      (student) => student.real_name === studentName
    );
    if (studentExists) {
      // 自动选择学员
      growthForm.value.student_name = studentName;
    } else {
      // 学员不存在或状态异常，显示错误提示
      alert('该学员状态异常，无法添加成长值');
    }
  }

  // 处理showRecord参数，自动打开记录弹窗
  const showRecord = route.query.showRecord as string;
  if (showRecord === 'true') {
    // 延迟打开弹窗，确保students数组已加载完成
    setTimeout(() => {
      showSingleAddModal.value = true;
    }, 100);
  }
...
// 监听路由变化，当路由切换到该页面时重新获取标签列表
watch(
  () => route.path,
  (newPath) => {
    if (newPath === '/teacher/growth') {
      fetchTags();
    }
  }
);
...
const fetchClassLeaderboard = async () => {
  try {
    console.log('开始获取班级排行榜数据');
    console.log('班级列表:', classes.value);
    
    if (classes.value.length === 0) {
      console.log('班级列表为空，先获取班级列表');
      await fetchClasses();
      console.log('获取班级列表后:', classes.value);
    }
    
    for (const cls of classes.value) {
      const classId = Number(cls.id);
      console.log(`获取班级 ${classId} ${cls.class_name} 的排行榜数据`);
      // 添加时间戳参数，防止浏览器缓存
      const timestamp = new Date().getTime();
      try {
        const response = await request.get(
          `/api/v1/leaderboard/class/${classId}?t=${timestamp}`
        );
        console.log(`班级 ${classId} 排行榜响应:`, response);
        
        // 处理不同的响应格式
        if (Array.isArray(response)) {
          classLeaderboards.value[classId] = response;
        } else if (response && response.items) {
          classLeaderboards.value[classId] = response.items;
        } else {
          classLeaderboards.value[classId] = [];
        }
        console.log(`班级 ${classId} 排行榜数据:`, classLeaderboards.value[classId]);
      } catch (error) {
        console.error(`获取班级 ${classId} 排行榜失败:`, error);
        classLeaderboards.value[classId] = [];
      }
    }
    
    console.log('所有班级排行榜数据:', classLeaderboards.value);
  } catch (error) {
...
const exportClassLeaderboard = async (classId: number, className: string) => {
  try {
    console.log('开始导出班级:', className, 'ID:', classId);
    ...
    if (!element) {
      console.error(
        '未找到排行榜元素，尝试使用ref获取:',
        classLeaderboardRefs.value[classId]
      );
      const refElement = classLeaderboardRefs.value[classId];
      if (!refElement) {
        alert('未找到排行榜元素，请刷新页面后重试');
        return;
      }
      element = refElement as HTMLElement;
    }

    console.log('找到排行榜元素:', element);
    ...
    title.textContent = `${className}班级成长值排行榜`;
    ...
    try {
      ...
      link.download = `${className}班级排行榜.png`;
...
const handleExport = async () => {
  ...
  if (selectedExportClass.value === 'all') {
    for (const cls of classes.value) {
      await exportClassLeaderboard(
        cls.id,
        `${cls.session || ''}届 ${cls.class_name}`
      );
    }
  } else {
    const selectedClass = classes.value.find(
      (cls) => cls.id === Number(selectedExportClass.value)
    );
    if (selectedClass) {
      await exportClassLeaderboard(
        selectedClass.id,
        `${selectedClass.session || ''}届 ${selectedClass.class_name}`
      );
    }
  }

  selectedExportClass.value = '';
};

// 成长日志相关函数
const fetchGrowthHistory = async () => {
  try {
    console.log('开始获取成长值变更流水');
    const params = new URLSearchParams();
    if (historyStartDate.value) {
      params.append(
        'start_time',
        new Date(historyStartDate.value).toISOString()
      );
      console.log('开始时间:', historyStartDate.value);
    }
    if (historyEndDate.value) {
      params.append('end_time', new Date(historyEndDate.value).toISOString());
      console.log('结束时间:', historyEndDate.value);
    }
    if (historyStudentName.value) {
      params.append('student_name', historyStudentName.value);
      console.log('学员姓名:', historyStudentName.value);
    }
    ...
    console.log('请求URL:', `/api/v1/growth/history?${params.toString()}`);
    const data: any = await request.get(
      `/api/v1/growth/history?${params.toString()}`
    );
    console.log('获取成长值变更流水响应:', data);
    console.log('响应类型:', typeof data);

    if (data && typeof data === 'object') {
      growthHistory.value = data?.items || [];
      historyTotal.value = data?.total || 0;
      console.log('成长值变更流水数据:', growthHistory.value);
      console.log('总记录数:', historyTotal.value);
    } else {
      growthHistory.value = [];
      historyTotal.value = 0;
      console.error('获取成长值变更流水失败: 响应数据格式错误');
    }
  } catch (error) {
